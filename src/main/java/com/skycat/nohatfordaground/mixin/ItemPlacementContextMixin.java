    ...
    private boolean stopPlacingCustomModelData(boolean original) {
        ...
        if (player != null) { // If it was a player
            if (player instanceof ServerPlayerEntity) { // If we're on the server side
                ...
                serverPlayer.networkHandler.sendPacket(new TitleS2CPacket(TitleS2CPacket.Action.ACTIONBAR, new LiteralText("You can't place that!"))); // Let the player know we're blocking it
                int slot;
                ...
                serverPlayer.networkHandler.sendPacket(new GuiSlotUpdateS2CPacket(-2, slot, stack)); // Send inventory update to the client. Magic number -2: The id that's used for /give and pick block. I don't know what it does.
            }
        }
